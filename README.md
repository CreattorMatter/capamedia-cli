...
### Release 0.43.0 — El autofix de env vars ya no rompe el Helm (y `CCC_PAYLOAD_MODE` se corrige)

- [x] v0.43.0 - **Fix doble en los helm.** (a) `CCC_PAYLOAD_MODE` llegaba en `FULL`/`PARTIAL` desde el scaffold (13 y 2 casos en los workspaces locales) y el autofix solo agregaba variables ausentes, nunca corregía un valor: ahora **reescribe el valor desviado** a `NONE` en los 3 ambientes (payload con PII en logs es un riesgo de seguridad), preservando `CCC_CUSTOM_LEVEL_DEBUG_ENABLED=true` solo en dev. (b) El injector insertaba `- name: ...` bajo `variables:`, que en los charts del MCP es un **mapping** (`variables.own.config`): dejaba los 3 helm con `ParserError` y, como los checks leen con regex, **el Check 7.8 daba PASS sobre un chart que Helm no renderiza**. Ahora inserta como hermano del primer item existente copiando su indentación, **auto-sana** charts ya corrompidos, y el **Check 7.12 nuevo (HIGH)** exige que los helm parseen como YAML (177/177 charts locales validados sin falsos positivos). +15 tests, suite 1146.

### Release 0.42.0 — `capamedia configurables`: fin del falso negativo del CSV

- [x] v0.42.0 - **Fix del falso negativo del CSV de configurables** (WSSeguridad0069). El archivo es ISO-8859-1 con delimitador `;`, así que un `grep` en locale UTF-8 sale con **código 1 y sin salida, indistinguible de "no encontrado"**; sumado al `|| echo "NO ENCONTRADO"` y a un `head -40`, el agente reportó `UMPSeguridad0087Config` como ausente teniendo 12 filas (con la `url` y el `ns` de Cyxtera DetectID). Nuevo `core/configurables.py` + comando **`capamedia configurables <Nombre>`** (`--json` de una línea, `--yaml`, `--variable`, `--exact`, `--limit`) con **contrato de exit codes**: `0` hay filas, `1` leído y ausente (definitivo), `2` ilegible (**sin respuesta, prohibido concluir**). Avisa de los 24 valores con acento corrupto en el origen. Canonicals corregidos: la columna real es `Configurable;Variable;Valor` (no `ConfigName`), el ejemplo `CMRCTEATR` no existía en el archivo, y se documentan 7868 filas / 533 configurables. +16 tests, suite 1131.

### Release 0.41.0 — Presupuesto de contexto: CLAUDE.md slim + prompts partidos

- [x] v0.41.0 - **Fix `Prompt is too long` en subagentes** (WSSeguridad0069). `init` ya no concatena los 22 canonicals en un `CLAUDE.md` de 202 KB: el archivo auto-cargado queda en ~22 KB con los canonicals núcleo y una tabla de **contexto bajo demanda** (`.capamedia/context/<name>.md`, uno por canonical, con "cuándo leerlo"). Los prompts > 48 KB (`migrate-rest-full`, 153 KB) se renderizan como índice + partes en `.capamedia/prompts/<name>/NN-*.md` (por `##`/`###`/`####`, ~24 KB cada una) que el agente lee bloque a bloque. Aplica a los 6 harnesses; presupuestos calibrados para **Opus** (modelo objetivo). `--inline-context` conserva el render anterior. `init` y `doctor` muestran el presupuesto. Router `/migrate` Paso 0 y agente `migrador` con reglas de lectura bajo demanda y un subagente por bloque. +11 tests, suite 1115.
