...
- [x] v0.44.0 - **`3.5.15` pasa a ser mínimo, no tope.** El agente bajaba a 3.5.15 un arquetipo que el MCP generó en 4.1.1 citando la Regla 8.5. Los checks ya aceptaban 4.1.1, así que era solo texto: corregido en los 8 canonicales del camino de `/migrate`, cero Python.
- [x] v0.44.0 - **Fuera los pins de `io.netty`.** Ya no se manda pinear el árbol de 13 módulos a `4.1.136.Final`: con el BOM de Spring Boot 4 es un downgrade y un pin manual se queda atrás al próximo CVE. Netty como runtime de WebFlux queda intacto.
- [x] v0.44.0 - **Librerías del banco para Spring Boot 4**: `lib-trace-logger-sb4:1.2.0`, `lib-event-logs-*:2.0.0`, `lib-bnc-api-client:3.0.0`. El autofix de la Regla 8 ya no baja una `3.x` a la versión del OLA y normaliza pre-releases; `validate_hexagonal` deja de exigir el literal `:1.1.0`, que ya rechazaba a los servicios OLA 2.
