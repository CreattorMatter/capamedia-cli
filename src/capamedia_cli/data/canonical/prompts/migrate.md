...
12. **Helm limpio:** en `helm/dev.yml`, `helm/test.yml` y `helm/prod.yml`, no puede quedar ningún placeholder `<...>` ni marcador `TODO/TBD/PENDIENTE/VALIDAR/REVISAR` en líneas activas; tampoco comentarios inline en líneas `name:`/`value:`. Si falta el valor real, reportarlo como handoff fuera del Helm.
13. **Pipeline/catalog namespace:** `metadata.namespace` en `catalog-info.yaml` debe ser `<prefijo>-middleware` según `metadata.name` (`csg-msa-sp-*` -> `csg-middleware`) y `KUBERNETES_NAMESPACE` en `azure-pipelines.yml` debe coincidir con ese valor.
