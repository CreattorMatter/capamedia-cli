...
* **Check 5.13: Paridad de short-circuit en ORQ (ORQ-RETURN-PARITY).** Solo `source_kind=orq`. En IIB, `CALL Proc() INTO respuesta; IF NOT respuesta THEN RETURN FALSE;` parece mandatorio, pero si el `PROCEDURE` invocado NO tiene `RETURN FALSE` en su rama de error (cae al `RETURN TRUE` final) el downstream es **best-effort** (su falla no corta el flujo legacy). El CLI cruza el `Mono.zip` migrado contra el `RETURN FALSE` de los `PROCEDURE` del ESQL legacy por **cardinalidades agregadas** (cuenta ramas con `.onErrorResume` —en el `ServiceImpl` o en sus `util/*Helper.java`— vs PROCEDURE best-effort), **no 1:1 por rama**: el legacy ORQ es secuencial y no expone una llave estable para parear rama↔procedure. Veredictos: fan-out **homogéneo todos-mandatory** con alguna rama `.onErrorResume` -> **FAIL HIGH** (más permisivo, ignora errores que el legacy propaga); **homogéneo todos-best-effort** sin ningún `.onErrorResume` -> **FAIL HIGH** (más estricto, rompe casos productivos, ej. cliente sin asesor → 500 en vez de 200); **mixto** (mandatory + best-effort) -> **FAIL MEDIUM** si las cardinalidades delatan permisividad (más ramas onError que best-effort) o estrictez (ningún onError), si no **FAIL LOW** (revisión manual — el check no distingue por rama, nunca HIGH automático en mixto); coherente -> **PASS**. Sin legacy -> **FAIL LOW**. Casos: ORQClientes0022 (OP21, best-effort), ORQClientes0023 (4 downstreams mandatory).
...
* **Check 8.7: Sin pins de io.netty (con excepción WebFlux).** Evitar declarar pins manuales de dependencias transitivas de Netty (`io.netty:*`) en el `dependencyManagement` de `build.gradle`. Pins manuales -> **FAIL HIGH**. **Excepción oficial v0.27.0**: en proyectos WebFlux (`spring-boot-starter-webflux` presente) el pin `io.netty:*:4.1.133.Final` está permitido (CVE-fix Snyk 2026-05 aprobado). Cualquier otra versión sigue bloqueada (incluida 4.2.x, que rompe Reactor Netty: `StacklessClosedChannelException`). MVC/SOAP: no se permite pin manual de ninguna versión.
* **Check 8.8: Árbol Netty completo en WebFlux (12 módulos, doble pin).** Cuando un proyecto WebFlux ya pinea `io.netty:*:4.1.133.Final`, los **12 módulos core** del árbol (`netty-common`, `netty-buffer`, `netty-transport`, `netty-resolver`, `netty-resolver-dns`, `netty-codec`, `netty-codec-dns`, `netty-codec-http`, `netty-codec-http2`, `netty-codec-socks`, `netty-handler`, `netty-handler-proxy`) deben estar pineados con **doble mecanismo**: `dependencyManagement { dependency '...' }` **y** `configurations.all { resolutionStrategy { force '...' } }`. El BOM de Spring Boot 3.5.x trae Netty en versión vulnerable (`4.1.121.Final`) y `dependencyManagement` no siempre gana sobre transitivas (lib-bnc, el propio BOM); el `force` lo garantiza. Pinear solo `netty-codec*` deja transitivos como `netty-handler-proxy` vulnerables (WSClientes0013: 9 CVEs). Faltantes -> **FAIL HIGH** (autofix `fix_netty_full_tree_pin`). Excluye binarios nativos (`netty-transport-native-*`) y `netty-tcnative-*`. **NO bumpear a 4.2.x** (rompe Reactor Netty).
