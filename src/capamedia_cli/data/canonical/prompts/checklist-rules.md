...
* **Check 5.13: Paridad de short-circuit en ORQ (ORQ-RETURN-PARITY).** Solo `source_kind=orq`. En IIB, `CALL Proc() INTO respuesta; IF NOT respuesta THEN RETURN FALSE;` parece mandatorio, pero si el `PROCEDURE` invocado NO tiene `RETURN FALSE` en su rama de error (cae al `RETURN TRUE` final) el downstream es **best-effort** (su falla no corta el flujo legacy). El CLI cruza las ramas del `Mono.zip` migrado contra el `RETURN FALSE` de los `PROCEDURE` del ESQL legacy: **(a)** PROCEDURE con `RETURN FALSE` + rama sin `.onErrorResume` -> **PASS**; **(b)** con `.onErrorResume` -> **FAIL HIGH** (migrado más permisivo, ignora errores que el legacy propaga); **(c)** PROCEDURE solo `RETURN TRUE` + rama best-effort con `.onErrorResume(Mono.just(EMPTY))` -> **PASS**; **(d)** solo `RETURN TRUE` + rama mandatoria sin `.onErrorResume` -> **FAIL HIGH** (migrado más estricto, rompe casos productivos, ej. cliente sin asesor → 500 en vez de 200 con campos vacíos). Sin legacy para verificar -> **FAIL LOW** (revisar manual). Caso: ORQClientes0022 (PROCEDURE OP21).
