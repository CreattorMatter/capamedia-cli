...
* **Check 0.6: cURL por operación WSDL en README/docs.** (Hallazgo TO 2026-08-25 §1.3.) `README.md` o `docs/**/*.md` debe contener al menos un bloque de código con `curl` por **cada** `<wsdl:operation>` del portType (mismas operaciones que el Check 0.3), nombrando la operación en el envelope/SOAPAction. Cada cURL lleva envelope SOAP de ejemplo (headerIn + bodyIn), `Content-Type: text/xml;charset=utf-8`, la respuesta de éxito (`error.codigo=0`, `tipo=INFO`) y al menos un caso de error de negocio (`tipo=ERROR`); con `invocaBancs=true` sumar el header sin `<bancs>` (`9927`, `tipo=FATAL`). Es insumo de los consumidores y **gate previo a asignar la tarjeta al TO**. Operaciones sin cURL -> **FAIL MEDIUM**.
...
* **Check 2.6: Logs INFO diagnósticos deben ser DEBUG.** Los logs de nivel INFO deben estar estrictamente acotados a inicio, fin de transacciones o eventos de contrato de negocio, e identificar **inequívocamente** la transacción (guid / requestId). Hallazgo TO 2026-08-25 §1.1: `"Request received for operation: {}"` e `"Input validation passed for: {}"` no aportan al monitoreo. Un `log.info(...)` / `CustomLogLevel.INFO` cuyo mensaje contenga `Request received`, `Input validation passed`, `Validation passed` o `Received request` -> **FAIL MEDIUM**: bajar a `log.debug(...)` (`ServiceLogHelper`), activable en desarrollo con `CCC_CUSTOM_LEVEL_DEBUG_ENABLED` / `TPL_LOG_DEBUG`.
* **Check 2.7: No abuso de log.info.** Evitar inyectar logs informativos redundantes o dentro de bucles interactivos intensivos.
* **Check 2.10: Sondas de management fuera del trace-logger (`TraceLoggerManagementPathConfig`).** Regla 9e.3 de `bank-official-rules.md`. `ServletRequestInformationExtractor` / `ReactiveRequestInformationExtractor` de `lib-trace-logger` vuelcan cada request en un `RequestInformationContextHolder` **singleton**: las sondas liveness/readiness/prometheus pisan el contexto del request de negocio (el TO vio `"type":"TRANSACTIONAL"` con `requestUri=/actuator/health/readiness`). Un filtro extra no lo evita: debe existir `infrastructure/config/TraceLoggerManagementPathConfig.java` que implemente `BeanPostProcessor` + `EnvironmentAware`, envuelva el bean por `instanceof ServletRequestInformationExtractor` (MVC: WAS, SOAP, BUS sin BANCS) o `instanceof ReactiveRequestInformationExtractor` (WebFlux: ORQ, BUS + invocaBancs) y deje pasar sin capturar los paths bajo `management.endpoints.web.base-path` (default `/actuator`; base-path en blanco cae a `/actuator`). Ausente, sin `BeanPostProcessor`, fuera de `infrastructure/config/` o **variante equivocada para el stack** (`_project_uses_webflux`) -> **FAIL HIGH** (autofix `fix_add_trace_logger_management_config` crea la clase y su test según el stack). Compila igual con `lib-trace-logger:1.4.0` (SB3) y `lib-trace-logger-sb4:1.2.0` (SB4).
* **Check 2.11: Test de `TraceLoggerManagementPathConfig`.** `src/test/java/**/TraceLoggerManagementPathConfigTest.java` con **≥ 6** métodos `given*_when*_then*` (JUnit 5 + Mockito + AssertJ, sin `@DisplayName` ni JavaDoc): `/actuator/prometheus`, `/actuator/health/liveness` y `/actuator/health/readiness` saltan al `chain` (delegate nunca invocado); request de negocio delega; base-path custom `/management`; base-path `""` cae a `/actuator`; bean ajeno no se envuelve (`isSameAs`); MVC además context path `/svc`. Utilidades: MVC `MockHttpServletRequest` + `mock(FilterChain)`; WebFlux `MockServerWebExchange` + `mock(WebFilterChain)` + `StepVerifier`. Ausente o con menos casos -> **FAIL MEDIUM**.
...
* **Check 7.8: Helm env vars trace-logger + payload.** Cada helm por-entorno (dev/test/prod) debe declarar las 7 env vars CCC_* del trace-logger con el valor esperado del ambiente: `CCC_CUSTOM_LEVEL_DEBUG_ENABLED = true` solo en `dev` (test/prod = `false`); `CCC_PAYLOAD_MODE = NONE` en los 3 (no loguear payload/PII); el resto igual en los 3. Faltantes o valor incorrecto -> **FAIL HIGH**. El autofix `fix_trace_logger_helm` inyecta las ausentes **y corrige los valores desviados** (cada var tiene un único valor válido por ambiente): `CCC_PAYLOAD_MODE` llega en `FULL`/`PARTIAL` desde el scaffold en varios servicios y eso significa **payload con PII en los logs**, así que se reescribe a `NONE` en los 3 en vez de solo reportarse. Inserta siempre dentro de la lista de env vars existente (`variables.own.config` en los charts del MCP) — ver Check 7.12. El log transaccional (`lib-event-logs`, `spring.kafka`) sigue siendo ORQ-only y NO se valida aca.
* **Check 7.10: Helm probes a `/actuator/health/liveness` y `/actuator/health/readiness`.** (Spring Boot 4, correo BPTPSRE 2026-08; Regla 9h.1.) En `helm/dev.yml`, `test.yml` y `prod.yml` el `livenessProbe` debe hacer `curl` a `/actuator/health/liveness` y el `readinessProbe` a `/actuator/health/readiness`; el agregado `/actuator/health` sin sufijo (o los paths cruzados) es error. Se aceptan las dos formas de shell: la canónica `if ! curl -s <url> | grep -q '"status":"UP"'; then exit 1; fi` (robusta al orden de campos / pretty-print) y la forma `cut` que emite el MCP `v20260827161016` (equivalente). Conservar `initialDelaySeconds` / `periodSeconds` / `timeoutSeconds` del chart. Desviación -> **FAIL HIGH** si el plugin Spring Boot es `>= 4`, **FAIL MEDIUM** en 3.5.x (ambas rutas funcionan; migrar en el PR de librerías). Autofix `fix_helm_probe_paths` reescribe solo el path dentro de cada bloque de probe.
* **Check 7.12: Helm por-entorno parsea como YAML.** Red de seguridad estructural: `helm/dev.yml`, `test.yml` y `prod.yml` deben cargar con un parser YAML. Un values file inválido **no lo renderiza Helm** y el deploy falla. No es teórico: hasta v0.42.0 el autofix de env vars insertaba `- name: ...` debajo de `variables:`, que en los charts del MCP es un **mapping** (`variables.own.config`), dejando una secuencia mezclada dentro de un mapping — los 3 helm de WSSeguridad0069 quedaron con `ParserError` y, como el resto de los checks lee valores con regex y no con el parser, el **Check 7.8 seguía dando PASS** (la herramienta rompía el chart y lo declaraba sano). Archivo que no parsea -> **FAIL HIGH**, y el 7.8 también falla con `YAML invalido: <error>`. Corrección: mover las env vars a la lista `variables.own.config`; `fix_trace_logger_helm` y `fix_helm_probes_enabled_env` ahora **auto-sanan** un chart ya corrompido (sacan la entrada mal ubicada y la re-insertan en la lista correcta) y nunca tocan un chart que parsea bien.
* **Check 7.11: `CCC_ACTUATOR_HEALTH_PROBES_ENABLED: "true"` en los 3 Helm.** Prerrequisito de 7.10: los endpoints `/actuator/health/{liveness,readiness}` solo existen con `management.endpoint.health.probes.enabled: ${CCC_ACTUATOR_HEALTH_PROBES_ENABLED}` en `application.yml` y el valor explícito `"true"` en `dev/test/prod` (Spring lo activa solo si detecta el cluster; el valor explícito evita depender de la detección). Faltante o distinto de `true` -> **FAIL MEDIUM** (autofix `fix_helm_probes_enabled_env` inyecta la env var ausente; un valor distinto se revisa a mano).
* **Check 7.9: WebClient oficial para downstream WS (Builder + ConnectionProvider).** Solo aplica a proyectos WebFlux que construyen `WebClient.builder(` manualmente (ORQ/BUS invocando microservicios WS downstream; BANCS via lib-bnc y Stratio quedan fuera). Exige el patrón oficial del banco (canonical `log-transaccional-orq.md` LT-3b): `ConnectionProvider` configurado (pool con maxConnections/pendingAcquire*), `.responseTimeout(...)` en vez de `ReadTimeoutHandler` (patrón legacy), y bloque top-level `webclient:` en `application.yml` con una entrada por downstream (`url`/`timeout`/`read-timeout`/`max-connections`/`pending-acquire-max-count` via `${CCC_*}`; `max-idle-time` y `pending-acquire-timeout` se omiten — defaults del código). Desviaciones -> **FAIL MEDIUM** (sin autofix determinístico: la corrección es cirugía de código guiada por el prompt REST §4.17).
...
* **Check 8.1: Spring Boot Plugin Version — baseline `4.1.1`, NUNCA bajar.** (Correos BPTPSRE 2026-08: "todos los proyectos nuevos van en Spring Boot 4"; el MCP Fabrics `v20260827161016` ya emite `4.1.1`.) Fuente única `SPRING_BOOT_BASELINE_VERSION = "4.1.1"` y `SPRING_BOOT_LEGACY_BASELINE_VERSION = "3.5.15"` en `version_policy.py`. Tres estados: plugin `< 3.5.15` o `4.x < 4.1.1` -> **FAIL HIGH** (scaffold viejo); `3.5.15 <= v < 4` -> **FAIL MEDIUM** (línea SB3 aceptada **solo para proyectos existentes** — no romper el pasado; el upgrade a SB4 va en el mismo PR que sube librerías: 8.13, 8.14, 8.9, 7.10); `>= 4.1.1` -> **PASS**, incluida cualquier versión mayor que emita el MCP (**nunca se pide bajar**). Autofix `fix_spring_boot_plugin_version` sube **solo dentro de la misma línea mayor** (`3.5.14 -> 3.5.15`, `4.0.x -> 4.1.1`), actualiza `spring_boot_version` en `migration-context.json` y jamás salta `3.x -> 4.x` (cambia artifactIds y librerías: lo decide el migrador en un proyecto nuevo o un PR de librerías en uno existente). Un proyecto **nuevo** no debe quedar en 3.5.x.
...
* **Check 17.8: `logging.event.excluded-paths` excluye `/actuator/**`.** (ORQ; LT-2 pasa de "default documentado" a **MUST**.) `com.pichincha.common.filter.LoggingWebFilter` de `lib-event-logs` (1.0.1 y 2.0.0) **sí** emite un registro por request: sin excluir las sondas, liveness/readiness/prometheus aparecen como `"type":"TRANSACTIONAL"` (hallazgo TO 2026-08-25 §1.2). `application.yml` debe declarar bajo `logging.event`: `excluded-paths: /actuator/**,/health,/metrics,/prometheus` (forma escalar o lista YAML). Ausente -> **FAIL HIGH** (autofix `fix_event_logs_excluded_paths` lo inyecta bajo `logging.event`); presente sin `/actuator/**` -> **FAIL HIGH** (revisión manual, el autofix no pisa valores).
