...
* **Check 2.10: Sondas de management fuera del `lib-trace-logger`.** Obligatorio en **TODO** servicio migrado — no depende de si invoca BANCS, del origen (WAS/BUS/ORQ) ni de la cantidad de operaciones. Debe existir `infrastructure/config/TraceLoggerManagementPathConfig.java`: un `BeanPostProcessor` + `EnvironmentAware` que envuelve el extractor del `lib-trace-logger` y deja pasar sin capturar los paths bajo `management.endpoints.web.base-path` (default `/actuator`; si llega en blanco cae a `/actuator`). Motivo: `RequestInformationContextHolder` es un `@Component` **singleton**, así que cada sonda de liveness/readiness/prometheus pisa el contexto del request de negocio y el trace `@BpTraceable` reporta `requestUri=/actuator/...` (hallazgo del TO 2026-08-25). Bajar `logging.level` no alcanza y un filtro adicional no puede impedir que el filtro de la librería corra. **La variante la decide el starter de `build.gradle`**: `spring-boot-starter-webflux` → `instanceof ReactiveRequestInformationExtractor`; `spring-boot-starter-web`/`-web-services` → `instanceof ServletRequestInformationExtractor`. Ausente, sin `BeanPostProcessor`, fuera de `infrastructure/config/` o con la variante equivocada -> **FAIL HIGH**. Autofix `fix_add_trace_logger_management_config` la crea con la variante del stack (no sobreescribe una existente). Código completo en `/doublecheck` Paso 1.10.
