...
def _build_doublecheck_prompt(
    ...
    return (
        ...
        "migrado (`spring.application.name` o carpeta, ej. `csg-msa-sp-*` -> "
        ...
        "- Matriz BANCS obligatoria: solo `source_type=bus|iib` con `invocaBancs=true` "
        "puede tener `lib-bnc-api-client`, `BancsService`, `BancsClientHelper`, "
        "`bancs.webclients`, `CCC_BANCS_*` o `dependsOn: lib-bnc-api-client`. "
        "En WAS, ORQ y BUS sin BANCS, elimínalos si aparecen y no los agregues.\n"
        ...
        "`application/output/port` y tests `@SpringBootTest` detectables.\n"
